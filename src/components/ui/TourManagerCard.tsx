...
const TourManagerCard: React.FC<TourManagerProps> = ({
  ...
  return <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow overflow-hidden border border-gray-100">
      {(featured || topRated || newJoined) && <div className={`text-xs font-medium text-white px-3 py-1 ${featured ? 'bg-purple-600' : topRated ? 'bg-yellow-500' : 'bg-green-500'}`}>
      ...
      <div className="p-5 flex flex-col items-center">
        <div className="relative">
          ...
          {verified && <div className="absolute bottom-3 right-0 bg-blue-500 text-white p-1 rounded-full">
        ...
        <div className="flex w-full gap-2">
          <Link to={`/tour-leader/${id}`} className="flex-1 text-center border border-blue-600 text-blue-600 hover:bg-blue-50 font-medium py-2 px-4 rounded-lg text-sm transition-colors">
            View Profile
          </Link>
          <button onClick={handleFavoriteClick} className={`p-2 rounded-lg border ${isFavorite ? 'border-red-200 bg-red-50 text-red-500' : 'border-gray-200 hover:bg-gray-50 text-gray-500'} transition-colors`}>
