          ...
          {showMatchScore && matchPercentage > 0 && (
            <div className="absolute top-3 right-3 bg-white rounded-full shadow-md p-1.5">
              <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center">
                <span className="text-blue-700 font-bold text-sm">{matchPercentage}%</span>
              </div>
            </div>
          )}
          {availability.includes('spot') && parseInt(availability) <= 3 && (
            ...
            {personality && personality.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {personality.slice(0, 3).map((trait, index) => (
                  <span
                    key={`${id}-trait-${index}`}
                    className={`text-xs px-2 py-0.5 rounded-full ${userPreferences.includes(trait) ? 'bg-blue-100 text-blue-800' : 'bg-gray-50 text-gray-600'}`}
                  >
                    {trait}
                  </span>
                ))}
                {personality.length > 3 && (
                  <span className="text-xs text-gray-500">+{personality.length - 3} more</span>
                )}
              </div>
            )}
          </div>
