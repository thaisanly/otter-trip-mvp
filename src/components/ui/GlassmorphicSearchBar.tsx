...
import { SearchIcon, MapPinIcon, CalendarIcon, UsersIcon, ChevronRightIcon, GlobeIcon, ArrowRightIcon } from 'lucide-react';
type GlassmorphicSearchBarProps = {
  className?: string;
};
const GlassmorphicSearchBar = ({
  className = ''
}: GlassmorphicSearchBarProps) => {
  ...
  return <div ref={searchBarRef} className={`
        ${className}
    ...
    boxShadow: isHovering ? '0 12px 32px rgba(0, 0, 0, 0.2)' : isScrolled ? '0 4px 12px rgba(0, 0, 0, 0.1)' : '0 8px 24px rgba(0, 0, 0, 0.15)',
    ...
    transform: `translate(-50%, ${isHovering && !isScrolled ? '-4px' : '0'})`
      ...
      <form onSubmit={handleSubmit} className="h-full w-full flex lg:flex-row flex-col">
        ...
        {!isScrolled && <div className="absolute left-4 top-1/2 transform -translate-y-1/2 lg:block hidden">
            <div className="relative w-[100px] h-[36px] rounded-full bg-white/35 flex items-center" style={{
          background: 'rgba(255, 255, 255, 0.35)'
        }}>
              {/* Sliding indicator */}
              <div className="absolute w-[50px] h-[28px] rounded-full bg-blue-600 transition-all duration-180 ease-in-out" style={{
            left: searchMode === 'experts' ? '4px' : '46px',
            background: 'linear-gradient(90deg, #4F7EFF 0%, #6D8BFF 100%)'
          }}></div>
              {/* Toggle buttons */}
              <button type="button" className={`relative z-10 w-[50px] h-full text-xs font-medium transition-colors ${searchMode === 'experts' ? 'text-white' : 'text-gray-700'}`} onClick={() => setSearchMode('experts')}>
                Experts
              </button>
              <button type="button" className={`relative z-10 w-[50px] h-full text-xs font-medium transition-colors ${searchMode === 'tours' ? 'text-white' : 'text-gray-700'}`} onClick={() => setSearchMode('tours')}>
                Tours
              </button>
            </div>
          </div>}
        ...
        `}>
          ...
          `}>
            ...
            <input type="text" placeholder="Where to?" className={`
                ...
                ${isScrolled ? 'text-sm' : ''}
              `} value={location} onChange={e => setLocation(e.target.value)} onFocus={() => setActiveInput('location')} onBlur={() => setActiveInput(null)} onKeyDown={handleKeyDown} />
          </div>
          {/* Date field */}
          <div className={`
            lg:flex-1 h-full lg:border-r border-white/35 relative
          ...
          `}>
            ...
            <input type="text" placeholder="When?" className={`
                w-full h-full bg-transparent border-0 outline-none
                lg:pl-12 pl-5 lg:pr-4 pr-5 text-gray-800 placeholder-gray-500
                ${activeInput === 'date' ? 'placeholder-opacity-70' : ''}
                ${isScrolled ? 'text-sm' : ''}
              `} value={date} onChange={e => setDate(e.target.value)} onFocus={() => setActiveInput('date')} onBlur={() => setActiveInput(null)} onKeyDown={handleKeyDown} />
          </div>
          ...
          `}>
            <div className="absolute left-5 top-1/2 transform -translate-y-1/2 text-gray-400">
              {searchMode === 'experts' ? <GlobeIcon size={isScrolled ? 18 : 20} /> : <UsersIcon size={isScrolled ? 18 : 20} />}
            </div>
            <input type="text" placeholder={searchMode === 'experts' ? 'Language' : 'Guests'} className={`
                w-full h-full bg-transparent border-0 outline-none
                lg:pl-12 pl-5 lg:pr-4 pr-5 text-gray-800 placeholder-gray-500
                ${activeInput === 'third' ? 'placeholder-opacity-70' : ''}
                ${isScrolled ? 'text-sm' : ''}
              `} value={searchMode === 'experts' ? language : guests} onChange={e => searchMode === 'experts' ? setLanguage(e.target.value) : setGuests(e.target.value)} onFocus={() => setActiveInput('third')} onBlur={() => setActiveInput(null)} onKeyDown={handleKeyDown} />
          </div>
          {/* Search button */}
          ...
          background: `linear-gradient(${isHovering ? '100deg' : '90deg'}, #4F7EFF 0%, #6D8BFF 100%)`
            ...
            <ArrowRightIcon size={isScrolled ? 18 : 20} className="transition-transform duration-200" style={{
