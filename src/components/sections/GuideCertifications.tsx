...
const GuideCertifications = ({
  ...
  return <div className={`bg-white rounded-xl shadow-sm p-6 ${className}`}>
      ...
      {certifications.length > 0 ? <div className="space-y-4">
          {certifications.map(cert => <div key={cert.id} className="flex items-start">
              <div className="mr-3 mt-0.5">{renderIcon(cert.icon)}</div>
              <div>
                <div className="flex items-center">
                  <h4 className="font-medium text-gray-900">{cert.name}</h4>
                  {cert.verified && <span className="ml-2 bg-blue-100 text-blue-800 text-xs px-2 py-0.5 rounded-full">
                      Verified
                    </span>}
                </div>
        ...
        </div> : <p className="text-gray-600">No certifications listed yet.</p>}
    </div>;
};
