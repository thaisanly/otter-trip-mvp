  ...
  return <div className="bg-gray-50 py-16">
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-end mb-10">
          <div>
            ...
            <p className="text-xl text-gray-600">
              Meet our top-rated local experts
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {featuredLeaders.map(leader => <TourLeaderCard key={leader.id} id={leader.id} name={leader.name} image={leader.image} location={leader.location} rating={leader.rating} reviewCount={leader.reviewCount} specialties={leader.specialties} languages={leader.languages} price={leader.price} availability={leader.availability} />)}
        </div>
      </div>
    </div>;
};
