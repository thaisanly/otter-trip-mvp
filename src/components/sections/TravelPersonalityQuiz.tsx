...
import { ChevronRightIcon, CheckIcon, ArrowLeftIcon, ArrowRightIcon, RefreshCwIcon, ShareIcon, MessageCircleIcon, StarIcon, ClockIcon, MapPinIcon, GlobeIcon, UserIcon, UsersIcon, ShoppingBagIcon, CameraIcon, UtensilsIcon, HeartIcon } from 'lucide-react';
...
const personalityTypes: PersonalityType[] = [{
  ...
  matches: [{
    ...
    id: '2',
    name: 'Miguel Santos',
    image: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?ixlib=rb-4.0.3&auto=format&fit=crop&w=128&q=80',
  ...
  matches: [{
    ...
    id: '6',
    name: 'Emma Chen',
    image: 'https://images.unsplash.com/photo-1580489944761-15a19d654956?ixlib=rb-4.0.3&auto=format&fit=crop&w=128&q=80',
...
const TravelPersonalityQuiz = () => {
  ...
  const toggleMultiSelection = (questionId: string, optionId: string) => {
    ...
    if (currentSelections.includes(optionId)) {
      handleAnswer(questionId, currentSelections.filter(id => id !== optionId));
    } else {
  ...
  return <div className="bg-blue-50 py-16">
      <div className="container mx-auto px-4">
        {/* Landing Section */}
        {!quizStarted && !showResults && <div className="max-w-5xl mx-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2">
                <div className="p-8 md:p-12 flex flex-col justify-center">
                  ...
                  <p className="text-lg text-gray-600 mb-8">
                    Take our 2-minute quiz to find Travel Pros who match your
                    unique travel style and preferences.
                  </p>
                  <div className="space-y-4 mb-8">
                    ...
                    <div className="flex items-center">
                      ...
                      <p className="text-gray-700">
                        Get matched with experts who fit your style
                      </p>
                    </div>
                    <div className="flex items-center">
                      <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center mr-4">
                  ...
                  <button onClick={startQuiz} className="bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-6 rounded-lg transition-colors inline-flex items-center justify-center">
                    Start Quiz
                    <ChevronRightIcon size={20} className="ml-2" />
                  </button>
                </div>
                <div className="hidden md:block relative">
                  <img src="https://images.unsplash.com/photo-1501785888041-af3ef285b470?ixlib=rb-4.0.3&auto=format&fit=crop&w=1000&q=80" alt="Travel personalities" className="w-full h-full object-cover" />
        ...
        {/* Quiz Interface */}
        {quizStarted && !showResults && <div ref={quizRef} className="max-w-4xl mx-auto">
            <div className="bg-white rounded-xl shadow-lg overflow-hidden">
              {/* Progress Bar */}
              <div className="w-full h-2 bg-gray-100">
                <div className="h-2 bg-blue-600 transition-all duration-500 ease-out" style={{
              width: `${progress}%`
            }}></div>
              </div>
              <div className="p-6 md:p-8">
                ...
                <div className="flex justify-between items-center mb-6">
                  <button onClick={handleBack} className={`flex items-center text-gray-600 hover:text-gray-900 transition-colors ${currentStep === 0 ? 'invisible' : ''}`}>
                ...
                <div className={`transition-opacity duration-400 ${isAnimating ? 'opacity-0' : 'opacity-100'}`}>
                  ...
                  {quizQuestions[currentStep].description && <p className="text-gray-600 mb-6">
                      {quizQuestions[currentStep].description}
                    </p>}
                  {/* Options */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                    {quizQuestions[currentStep].options.map(option => <div key={option.id} className={`
                    ...
                    handleAnswer(quizQuestions[currentStep].id, option.id);
                        ...
                        <div className="flex items-start">
                          <div className="mr-4 mt-1">
                            {isMultiSelect() ? <div className={`w-5 h-5 rounded border flex items-center justify-center ${isOptionSelected(quizQuestions[currentStep].id, option.id) ? 'bg-blue-600 border-blue-600' : 'border-gray-300'}`}>
                                {isOptionSelected(quizQuestions[currentStep].id, option.id) && <CheckIcon size={12} className="text-white" />}
                              </div> : <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${isOptionSelected(quizQuestions[currentStep].id, option.id) ? 'bg-blue-600 border-blue-600' : 'border-gray-300'}`}>
                                {isOptionSelected(quizQuestions[currentStep].id, option.id) && <div className="w-2 h-2 rounded-full bg-white"></div>}
                              </div>}
                          </div>
                          <div className="flex-1">
                            ...
                            {option.description && <p className="text-sm text-gray-600">
                                {option.description}
                              </p>}
                          </div>
                        </div>
                  ...
                  {isMultiSelect() && <div className="flex justify-end">
                      <button onClick={handleNext} disabled={!hasValidAnswer()} className={`
                  ...
                  {!isMultiSelect() && currentStep === quizQuestions.length - 1 && hasValidAnswer() && <div className="flex justify-end mt-6">
                        <button onClick={calculateResults} className="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2.5 px-6 rounded-lg flex items-center transition-colors">
                          See Results
                          <ArrowRightIcon size={16} className="ml-2" />
                        </button>
                      </div>}
                  {/* Skip option */}
                  {!isMultiSelect() && !hasValidAnswer() && <div className="text-center">
                      <button onClick={handleNext} className="text-gray-500 hover:text-gray-700 text-sm font-medium">
                        Skip this question
                      </button>
                    </div>}
                </div>
        ...
        {/* Results Page */}
        {showResults && personalityResult && <div ref={resultsRef} className="max-w-5xl mx-auto">
            <div className="bg-white rounded-xl shadow-lg overflow-hidden mb-8">
              ...
              <div className="p-6 md:p-8">
                <div className="mb-8">
                  ...
                  <div className="bg-blue-50 border border-blue-100 rounded-lg p-4">
                    <h4 className="font-medium text-gray-900 mb-3">
                      Your Key Travel Traits
                    </h4>
                    <div className="flex flex-wrap gap-2">
                      {personalityResult.traits.map((trait, index) => <span key={index} className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm">
                          {trait}
                        </span>)}
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {personalityResult.matches.map(expert => <div key={expert.id} className="border border-gray-200 rounded-xl overflow-hidden hover:shadow-md transition-all">
                        <div className="relative">
                          <img src={expert.image} alt={expert.name} className="w-full h-32 object-cover" />
                        ...
                        <div className="p-4">
                          ...
                          <div className="flex flex-wrap gap-1 mb-4">
                            {expert.specialties.slice(0, 2).map((specialty, index) => <span key={index} className="bg-gray-100 text-gray-700 text-xs px-2 py-0.5 rounded">
                                  {specialty}
                                </span>)}
                            {expert.specialties.length > 2 && <span className="bg-gray-100 text-gray-700 text-xs px-2 py-0.5 rounded">
              ...
              <div className="border-t border-gray-200 p-6 flex flex-wrap gap-4 justify-between items-center">
                <div className="flex space-x-3">
                  ...
                  <button onClick={resetQuiz} className="flex items-center text-gray-700 hover:text-gray-900">
