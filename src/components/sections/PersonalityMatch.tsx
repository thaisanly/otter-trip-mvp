...
const PersonalityMatch = ({
  ...
  const calculateMatchPercentage = () => {
    ...
    const matchCount = tourLeaderPersonality.filter(trait => userPreferences.includes(trait)).length;
  ...
  return <div className={`bg-white rounded-xl shadow-sm p-6 ${className}`}>
      <div className="flex justify-between items-center mb-4">
        ...
        {showMatchPercentage && userPreferences.length > 0 && <div className="flex items-center">
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        ...
        return <InterestTag key={trait} label={traitInfo.label} icon={<span className="text-lg">{traitInfo.emoji}</span>} selected={isMatch} className={isMatch ? 'border-green-600 bg-green-50 text-green-800' : ''} />;
      })}
      </div>
      {tourLeaderPersonality.length > 5 && <button onClick={() => setShowAll(!showAll)} className="text-blue-600 text-sm font-medium hover:text-blue-800">
          {showAll ? 'Show less' : `Show all (${tourLeaderPersonality.length})`}
        </button>}
      {userPreferences.length === 0 && <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-100 text-sm text-gray-700">
          <p>
            Set your personality preferences in your profile to see how well you
            match with this guide.
          </p>
        </div>}
    </div>;
};
